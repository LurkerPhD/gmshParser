...
std::pair<bool, std::string> parseName(const std::string &_src,
...
class gmshParser::impl {
 ...
 public:
    ...
    for (auto EntityTag : EntityTags) {
      ...
      for (std::size_t i = 0, nn = nodeTags.size(); i < nn; ++i) {
        ...
        for (XMLElement *_node = _nodeEntry->FirstChildElement("node"); _node;
             _node = _node->NextSiblingElement("node"))
  ...
  void addBoundaries(XMLNode *root) {
    ...
    for (auto PhysicalTag : PhysicalTags) {
      ...
      if (_isBoundary1) {
        ...
        for (auto _phase = root->FirstChildElement("phase"); _phase;
             _phase = phase->NextSiblingElement("phase"))
          if (_phase->IntAttribute("id") == phase_ID) phase = _phase;
        if (!phase) {
        ...
        XMLElement *_boundaryCondition =
            phase->FirstChildElement("boundary_condition");

        gmsh::model::mesh::getNodesForPhysicalGroup(
            PhysicalTag.first, PhysicalTag.second, nodeTags, coords);
        for (int bcNode : nodeTags) {
          XMLElement *newbc =
              addChildElement(_boundaryType.c_str(), _boundaryCondition);
  ...
  void addMaterial(XMLNode *root, const std::string &type, const int &id) {
    for (auto _mat = root->FirstChildElement("mat"); _mat;
         _mat = _mat->NextSiblingElement("mat"))
      if (id == _mat->IntAttribute("id")) return;
    ...
    if (type.find("DuncanChang") != std::string::npos) {
      ...
      XMLElement *normal_compress_slope =
          addChildElement("normal_compress_slope", _mat);
  ...
  void addDescribe(XMLNode *root) {
    ...
    if (m_dim == 2) describe->SetAttribute("plane_stress", false);
  ...
  void addOutput(XMLNode *root) {
    ...
    XMLElement *displacement_amplifier =
        addChildElement("displacement_amplifier", output);
  ...
  void addCalculationConfiguration(XMLNode *root) {
    ...
    XMLElement *parallel = addChildElement("parallel", calConfig);
    parallel->SetAttribute("active", true);
    ...
    XMLElement *time_step = addChildElement("time_step", calConfig);
    {
      ...
      XMLElement *time_differential =
          addChildElement("time_differential", time_step);
      time_differential->SetAttribute("theta", .8);
      XMLElement *large_defomation =
          addChildElement("large_defomation", time_step);
      large_defomation->SetAttribute("active", false);
    }
    ...
    XMLElement *gravity_calculation =
        addChildElement("gravity_calculation", calConfig);
  ...
  void addTopology(XMLNode *root) {
    ...
    XMLElement *element_mega_data =
        addChildElement("element_mega_data", topology);
    {
      ...
      XMLElement *Tetrahedral =
          addChildElement("Tetrahedral", element_mega_data);
  ...
  void addInitCondition(XMLNode *root) {
    ...
    XMLElement *init_displacement =
        addChildElement("init_displacement", init_condition);
    addChildComment(" <node=\"1\" dof=\"x1\" value=\"some_value\"/> ",
                    init_displacement);
